...
def search_food_handle(current_slot, customer_utterance, state_tracker_obj, entities, lac, db_obj, collection_name):
    ...
    if not last_slot_state:
        ...
        if not give_up_state:
            return give_up_nlg.robot_response_give_up()
    else:
        food_nlu.ie_all_search_food(customer_utterance, lac, entities)
